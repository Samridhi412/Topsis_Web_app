...
def send_mail(email_id,resultfile):
    fromaddr = "samridhi101916086@gmail.com"
    toaddr = email_id
...
if __name__ == '__main__':    
    ...
    st.write("""
    ## Topsis Score and rank generator by Samridhi Garg(101916086)
    """)
    st.write("""
    #### To get the result: pip install python-topsis-Samridhi-101916086==4.3.1
    """)
    try:
        spectra = st.file_uploader("upload file", type={"csv", "xlsx"})
        # print(spectra.name.split('.')[1])
        # print(spectra.name)
        if spectra is not None:
            ...
            if submit_button:
                try:
                    ...
                    if spectra.name.split('.')[1]!="csv" and spectra.name.split('.')[1]!="xlsx":
                        st.error("File format must be a csv of excel file")
                    if spectra.name.split('.')[1]=="csv":
                        spectra_df = pd.read_csv(spectra)
                    if resultfile.split('.')[1]!="csv":
                        st.error("Output File format must be a csv")    
                    elif spectra.name.split('.')[1]=="xlsx":
